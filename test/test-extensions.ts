...
@suite class TestExtensions {
    ...
    @test async "Test Reset"() {
...

        {
            ...
            const installing = this.extensionManager.installPackage(dni, false, 10000, (i) => i.Message.Subscribe((s, m) => { console.log(`Installer:${m}`) }));
...

        {
            ...
            // install/overwrite
            const dni = await this.extensionManager.findPackage("echo-cli", "*");
            const installing = this.extensionManager.installPackage(dni, true, 10000, (i) => i.Message.Subscribe((s, m) => { console.log(`Installer2:${m}`) }));

            // install at the same time?
            const dni2 = await this.extensionManager.findPackage("echo-cli", "*");
            const installing2 = this.extensionManager.installPackage(dni2, true, 10000, (i) => i.Message.Subscribe((s, m) => { console.log(`Installer3:${m}`) }));
